...
with recursive Hop as (
	...
	select 
		n + 1 as n, flight.id, flight.outbound, flight.inbound, 
		flight.s_dep, flight.s_arv
	from
		Hop cross join Flight
	where 
		n < (select n from n) and Hop.inbound = Flight.outbound 
		and (Flight.s_dep - Hop.s_arv) <= '24:00:00' 
		and (Flight.s_dep - Hop.s_arv) >= '00:00:00'
) 
